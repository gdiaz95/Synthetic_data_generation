...
synthetic data. It will loop 10 times, with each iteration training on the
output of the previous one, while always evaluating against the original data.
"""
...
def load_or_train_synthesizer(training_data, metadata, model_path, iteration):
    ...
    iterations, it trains a new CTGANSynthesizer and saves it.
    """
    # For the first iteration, check for a pre-trained model
    if iteration == 1 and os.path.exists(model_path):
        ...
        # For all other cases, train a new model
...
def main():
    ...
    for i in range(1, TOTAL_ITERATIONS + 1):
        ...
        # In the first iteration, we train on the original data.
        # In subsequent iterations, we train on the previously generated synthetic data.
        ...
        # Always evaluate and plot against the ORIGINAL dataset
