...
class NonParamGaussianCopulaSynthesizer:
    ...
    def __init__(self, enforce_min_max_values=True):
    ...
    def fit(self, data):
        ...
        self._model_state = self._learn_distributions_and_correlation(data)
        self._fitted = True
    ...
    def _learn_distributions_and_correlation(self, df):
        """
        Learns the marginal distributions and correlation matrix.
        ...
        for col in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(dtype):
                ...
                is_integer = False
                if len(valid_data) > 0:
                    if np.allclose(valid_data % 1, 0):
                        is_integer = True
                
                sorted_vals = np.sort(valid_data.values)
                ...
                u = self._empirical_cdf_continuous(series, rng)
            ...
            # --- B. Categorical Handling ---
                ...
                u = self._empirical_cdf_categorical(series, sorted_labels, rng)

            # Convert to Gaussian Z
            z_df[col] = self._uniform_to_gaussian(u)

        # Learn Correlation
        correlation_matrix = z_df.corr(method='pearson')
        
        # Repair non-PD correlation matrices immediately
    ...
    def _empirical_cdf_continuous(self, column, rng, integer_tolerance=1e-12):
        arr = np.asarray(column, float)
        mask_nan = np.isnan(arr)
        valid = arr[~mask_nan]
        n_valid = valid.size
        if n_valid == 0: return np.full(arr.shape, np.nan, dtype=float)
        ...
        if not is_integer_valued:
            ranks_valid = rankdata(valid, method='average')
            u[~mask_nan] = (ranks_valid - 0.5) / n_valid
            return u

        uniques, counts = np.unique(valid, return_counts=True)
        p = counts.astype(float) / counts.sum()
        P = np.cumsum(p)
        L = np.concatenate([[0.0], P[:-1]])
        ...
        u[~mask_nan] = np.clip(L[idx] + r * p[idx], 1e-12, 1 - 1e-12)
        return u
        
    def _empirical_cdf_categorical(self, column, sorted_labels, rng):
        arr = np.asarray(column, dtype=object)
        arr_filled = np.array(["<NaN>" if pd.isna(v) else v for v in arr], dtype=object)
        if "<NaN>" not in sorted_labels and pd.isna(column).any():
             labels = sorted_labels + ["<NaN>"]
        else:
             labels = sorted_labels

        if arr_filled.size == 0: return np.full(arr.shape, np.nan, dtype=float)
        ...
        if total == 0: return np.full(arr.shape, np.nan, dtype=float)
